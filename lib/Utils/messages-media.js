...
const encryptedStream = async (media, mediaType, { logger, saveOriginalFileIfRequired, opts, isPtt, forceOpus } = {}) => {
    ...
    const encBodyPath = (0, path_1.join)(getTmpFilesDirectory(), mediaType + (0, generics_1.generateMessageID)() + '.enc');
    ...
    try {
        ...
        return {
            mediaKey,
            encWriteStream: (0, fs_1.createReadStream)(encBodyPath),
            bodyPath,
            mac,
            fileEncSha256,
            fileSha256,
        ...
        try {
            await fs_1.promises.unlink(encBodyPath);
        }
        catch (err) {
        }
...
const getWAUploadToServer = ({ customUploadHosts, fetchAgent, logger, options }, refreshMediaConn) => {
    return async (stream, { mediaType, fileEncSha256B64, newsletter, timeoutMs }) => {
        ...
        if (Buffer.isBuffer(stream)) {
            reqBody = stream;
        } else if (stream instanceof fs_1.ReadStream || (stream.path && typeof stream.path === 'string')) {
            fileStreamPath = stream.path;
        } else {
            const tempPath = (0, path_1.join)(getTmpFilesDirectory(), `upload-${(0, generics_1.generateMessageID)()}`);
        ...
        for (const { hostname, maxContentLengthBytes } of hosts) {
            ...
            try {
                ...
                if (fileStreamPath) {
                    reqBody = (0, fs_1.createReadStream)(fileStreamPath);
                }
